...
extern "C" {
    ...
    int service_call_audio_start(unsigned int callid, const char *call_number)
    {
        TSDK_RESULT ret;
    ...
    int service_call_video_start(const char *call_number, unsigned int local_video_wnd, unsigned int remote_video_wnd)
    {
        int result;
        ...
        result = tsdk_set_video_window(call_id, SERVICE_CALL_D_VIDEO_WINDOW_NUM, video_window_info);
        if (TSDK_SUCCESS != result)
        {
